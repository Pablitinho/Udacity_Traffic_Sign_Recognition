...
with tf.Session(graph=graph_1) as session:
  ...
  print("Train accuracy: %.1f%%" % get_accuracy(X_train, Y_train, session, train, accuracy_op, labels_pred_softmax, batch_size))
  print("Valid accuracy: %.1f%%" % get_accuracy(X_valid, Y_valid, session, train, accuracy_op, labels_pred_softmax, batch_size))
